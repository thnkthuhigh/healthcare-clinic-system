...
export function ReceptionPage() {
  ...
  // Quick check-in by bookingId (paste from QR)
  ...
  // QR scanner state (dynamic import of html5-qrcode at runtime)
  const [scannerOpen, setScannerOpen] = useState(false);
  const [scannerLoading, setScannerLoading] = useState(false);
  const [scannerError, setScannerError] = useState<string | null>(null);
  // scannerRef removed (not used)

  const startScanner = async () => {
    setScannerError(null);
    setScannerLoading(true);
    try {
      // dynamic import; html5-qrcode types are not bundled here
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const mod = await import('html5-qrcode');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const Html5Qrcode = (mod as any).Html5Qrcode ?? (mod as any).default ?? mod;
      ...
      if (!container) {
        ...
        // append after the search area
        const parent = document.querySelector('#reception-page-root') ?? document.body;
      ...
      setScannerError(message || 'Không thể khởi tạo camera scanner. Hãy cài `html5-qrcode`');
    } finally {
  ...
  const stopScanner = async () => {
    try {
      ...
      console.debug('stopScanner error', e);
    }
    setScannerOpen(false);
  };
  ...
  return (
    <div id="reception-page-root" className="space-y-4">
      {/* Summary cards */}
      ...
      {activeTab === 'checkin' && (
        <div className="space-y-4">
          <div className="rounded-lg border border-slate-200 bg-white p-6">
              ...
              <div className="flex items-center gap-2">
                {!scannerOpen ? (
                  <button
                    ...
                    className="ml-2 inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                  >
                    {scannerLoading ? (
                  ...
                  <button
                    onClick={stopScanner}
                    className="ml-2 inline-flex items-center gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-100"
                  >
            ...
            {/* Quick bookingId check-in (mobile shows below search button) */}
